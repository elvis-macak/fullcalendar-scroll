...
MonthScrollView = BasicView.extend({ // make a subclass of View
  ...
  initialize: function() {
    ...
    this.rangeStart = date.clone().startOf(this.intervalUnit);
    this.rangeEnd = this.rangeStart.clone().add(this.intervalDuration);
    var that = this;
  },

	computeRange: function(date) {

		var intervalStart = this.rangeStart.clone();
		var intervalEnd = this.rangeEnd.clone();
	...
	computePrevDate: function(date) {
    this.rangeStart.add(-2, 'weeks');
    this.rangeEnd.add(-2, 'weeks');
	...
	numberCellHtml: function(cell) {
		var date = cell.start;
		var classes;
    var dateContent = date.date();

		if (!this.dayNumbersVisible) { // if there are week numbers but not day numbers
			return '<td/>'; //  will create an empty space above events :(
		}

    classes = this._getDayClasses(date);
		classes.unshift('fc-day-number');
    if (date.date() == 1) {
      dateContent = date.format();
      classes.push('fc-month-start');
    }
  ...
  _getDayClasses: function (date) {
		var classes = [ 'fc-' + dayIDs[date.day()] ];

		if (date.month()%2) {
			classes.push('fc-month-even');
    }else {
			classes.push('fc-month-odd');
		};
    return classes;
  },
	...
	headCellHtml: function(cell) {
		...
		return '' +
			'<th class="fc-day-header fc-' + dayIDs[date.day()] + '">' +
			dayIDs[date.day()] +
			'</th>';
	},
...
fcViews.monthScroll = {
  ...
  duration: { months: 6 }, // important for prev/next
	defaults: {
    titleFormat: 'YYYY-MM'
	}
};
$('#calendar .fc-view-container').scroll
;;
